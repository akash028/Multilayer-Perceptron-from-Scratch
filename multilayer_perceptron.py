...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        num = 2 if self.hidden_act_name == 'relu' else 1
        # 2 for 'He Initialization' and 1 for 'Xavier Initialization'

        self._h_weights = np.random.randn(self._X.shape[1], self.n_hidden) * np.sqrt(num/self._X.shape[1])
        self._h_bias = np.zeros((1, self.n_hidden))
        if self.hidden_act_name == 'relu':
            self._h_bias = np.where(self._h_bias == 0, 0.01, self._h_bias)

        # Initialize output weights and biases
        self._o_weights = np.random.randn(self.n_hidden, self._y.shape[1]) * np.sqrt(num/self.n_hidden)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iterations):
            #forward
            z_h = np.dot(self._X, self._h_weights) + self._h_bias
            A_h = self.hidden_activation(z_h) # Output for activation after hidden activation

            z_o = np.dot(A_h, self._o_weights) + self._o_bias
            A_o = self._output_activation(z_o) # Output for activation after output activation

            if i % 20 == 0:
                self._loss_history.append(cross_entropy(self._y, A_o))
            #backward propogation
            dZ_o = A_o - self._y
            dw_o = np.dot(A_h.T, dZ_o)

            db_o = np.sum(dZ_o, axis=0, keepdims=True)/n_samples

            dZ_h = np.dot(dZ_o, self._o_weights.T) * self.hidden_activation(z_h, True)
            dw_h = np.dot(dZ_h.T, X)/n_samples
            db_h = np.sum(dZ_h, axis=0, keepdims=True) / n_samples

            #revise weights and bias
            self._o_weights = self._o_weights - self.learning_rate * dw_o
            self._o_bias = self._o_bias - self.learning_rate * db_o
            self._h_weights = self._h_weights - self.learning_rate * dw_h.T
            self._h_bias = self._h_bias - self.learning_rate * db_h
    ...
    def predict(self, X):
        ...
        """
        z_h = np.dot(X, self._h_weights) + self._h_bias
        A_h = self.hidden_activation(z_h)
        z_o = np.dot(A_h, self._o_weights) + self._o_bias
        A_o = self._output_activation(z_o)

        y_pred = np.array([np.argmax(l) for l in A_o])
